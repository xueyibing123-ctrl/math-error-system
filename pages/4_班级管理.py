...
with tab1:
    ...
    if teacher_classes:
        students = [s for s in students if s[2] in teacher_classes]
        st.caption(f"当前显示：**{'、'.join(teacher_classes)}** 的学生")

    if not students:
        st.info("暂无学生注册，学生注册后会显示在这里。")
    else:
        df_stu = pd.DataFrame(students, columns=["ID","姓名","班级","注册时间"])
        available_classes = sorted(df_stu["班级"].dropna().unique().tolist())
        classes_filter = ["全部"] + available_classes
        selected_class = st.selectbox("按班级筛选", classes_filter)
        df_show = df_stu[df_stu["班级"] == selected_class] if selected_class != "全部" else df_stu

        col1, col2 = st.columns(2)
        with col1: st.metric("学生总数", len(df_show))
        with col2: st.metric("班级数", df_stu["班级"].nunique())
...
with tab2:
    ...
    if teacher_classes:
        all_classes = [c for c in all_classes if c in teacher_classes]

    if not all_classes:
        st.info("暂无班级数据，学生注册时填写班级后会显示在这里。")
    else:
        selected = st.selectbox("选择班级查看详情", all_classes)
        ...
        if not rows:
            ...
            with col1: st.metric("答题记录数", len(df))
            with col2: st.metric("参与学生数", df["学生姓名"].nunique())
            with col3:

...
# ── Tab 布局
if role == "admin":
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 班级总览", "📈 错因趋势", "🚨 预警管理", "📋 原始记录", "👤 账号管理"])
else:
...
if role == "admin":
    with tab5:
        ...
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("SELECT id, username, role, class_name, created_at FROM users ORDER BY role, class_name, username")
        ...
        st.divider()
        st.subheader("🗑️ 删除账号")
        usernames = [r["用户名"] for _, r in df_users.iterrows() if r["角色"] != "admin"]
        if usernames:
            del_user = st.selectbox("选择要删除的账号", usernames, key="del_user")
            if st.button("删除该账号", type="primary", key="btn_del"):
                conn = get_conn()
                cur = conn.cursor()
                cur.execute("DELETE FROM users WHERE username=%s AND role != 'admin'", (del_user,))
                conn.commit()
                cur.close()
                conn.close()
                st.success(f"已删除账号：{del_user}")
                st.rerun()
        else:
            st.info("暂无可删除的账号")
